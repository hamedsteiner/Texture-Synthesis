...
text=im2double(imread('text.jpg')); % M1 patch=108 100
...
psize=105    ;%patch size
ovlpsize=floor(psize/6);
tol=0.3;
%Method2


for out_row=1:psize-ovlpsize:out_height-(ovlpsize-1)
    remaining=out_height-(ovlpsize-1)-out_row
    for out_col=1:psize-ovlpsize:out_width-(ovlpsize-1)
...
figure,imshowpair(Image,Result, 'montage'),title("Original                    Method2 with patch= 45");        
