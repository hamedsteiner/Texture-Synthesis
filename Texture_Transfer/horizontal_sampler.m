function [y1,y2] = horizontal_sampler(in_height,in_width,psize,ovlpsize,Image,out_row,out_col,Result,target_image,errors_pnts)
    ...
    if ( out_row==1 )
        for i=1: step:in_height -psize+1
            for j=1: step:in_width -psize+1
                patch_ovlp=Image(i:i+psize-1,j:j+ovlpsize-1,:);
                        %analyze new patch from 1 to ovlp, analyze prev patch from col to prevj
                outimage_ovlp=Result( out_row:out_row+psize-1, out_col:out_col+ovlpsize-1, :);
                
                temp=(outimage_ovlp-patch_ovlp).*(outimage_ovlp-patch_ovlp); 
                errors_pnts(i,j)=sum(sum(sum(temp)));
            end
        end
        y2=zeros( (in_height -psize+1) ,(in_width -psize+1)); 
    else
        for i=1: step:in_height -psize+1
            for j=1: step:in_width -psize+1
                patch_ovlp=Image(i:i+psize-1,j:j+ovlpsize-1,:);
